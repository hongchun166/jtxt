...
/** 提交好友申请页面
* */
public class ApplyFriendActivity extends BaseActivity implements HttpRequestListener<BaseResult> {
    ...
    public void onSend() {
        showProgressDialog("");
        HttpServiceManager.addFriend(mFriend.account, mFriend.getName(),addFriendRequest);

//        HttpServiceManager.applyFriend(mSelt.account, mFriend.friendAccount, leaveMessageEdt.getText().toString().trim(), this);
    }
    ...
    public void onHttpRequestSucceed(BaseResult result, OriginalCall call) {
        hideProgressDialog();
        if (result.isSuccess()){
            ToastUtils.s(ApplyFriendActivity.this, getString(R.string.already_commit_apply_friend));
            setResult(RESULT_OK, getIntent());
            finish();
        }else {
            ToastUtils.s(ApplyFriendActivity.this, getString(R.string.commit_apply_friend_failure));
        }
    }
